...
export const ConnectionList: React.FC<ConnectionListProps> = ({
  ...
  const { selectedConnectionId, selectConnection, removeConnection } =
    useWebSocketStore();
  ...
  const handleDelete = useCallback(
    async (e: React.MouseEvent, id: string) => {
      e.stopPropagation();
      try {
        await fetch(`/__motia/websockets/connections/${id}`, {
          method: "DELETE",
        });
        removeConnection(id);
  ...
  if (connections.length === 0) {
    return (
      <Empty className="h-32">
        ...
        <EmptyDescription>
          Track connections using the API or create a test connection
        </EmptyDescription>
      </Empty>
    );
  }
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1.5">
              <Badge
                variant={statusVariants[connection.status]}
            ...
            <div className="text-[10px] text-muted-foreground/60 mt-1 truncate font-mono">
              {connection.id}
            </div>
          </div>
          <Button
