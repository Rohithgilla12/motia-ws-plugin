import { Button, Empty, EmptyDescription, EmptyTitle, cn } from "@motiadev/ui";
import {
  Check,
  ChevronDown,
  ChevronUp,
  ...
  const messages = streamFilter
    ? allMessages.filter((msg) => {
        if (typeof msg.data === "object" && msg.data !== null) {
          return (
            (msg.data as Record<string, unknown>).streamName === streamFilter
          );
        }
        return false;
      })
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        {messages.map((message, index) => {
          ...
          const msgStreamName: string | null =
            typeof message.data === "object" && message.data !== null
              ? String(
                  (message.data as Record<string, unknown>).streamName ?? ""
                )
              : null;
          const streamColor: StreamColor | undefined =
            msgStreamName && streamColors
              ? streamColors[msgStreamName]
              : undefined;
          ...
          if (isSent) {
            ...
            style = {
              ...
              label:
                msgStreamName?.replace("__motia.", "").replace(/-/g, " ") ||
                "Stream",
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5">
                <div className="flex items-center gap-2.5">
                  {/* Type Badge */}
                  <div
                    className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md ${style.labelBg}`}
                  >
                    <span className={`text-xs ${style.labelColor}`}>
                      {style.icon}
                    </span>
                    <span
                      className={`text-[10px] font-semibold uppercase tracking-wider ${style.labelColor}`}
                    >
                      {style.label}
                    </span>
                  </div>
        ...
        {messages.length === 0 && (
          <Empty className="h-64">
            ...
            >
              {streamFilter ? (
                <Radio
                  className={cn(
                    "w-7 h-7",
                    streamColors?.[streamFilter]?.text ||
            ...
            <EmptyTitle>
              {streamFilter ? "No messages in this stream" : "No messages yet"}
            </EmptyTitle>
            <EmptyDescription>
              {streamFilter
